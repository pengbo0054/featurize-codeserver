...
一个重新定义并优化的代码编辑器，专为构建和调试现代网络和云应用程序而设计。
"""
            )
    ...
    def installation(self, install_location):
        ...
        self.execute_command("wget https://code-server.dev/install.sh ｜ bash", self.cfg.source_directory)
        
        file_path = os.path.join(install_location, 'config.yaml')
    ...
    def close(self):
        ...
        self.execute_command(
            f"sudo systemctl stop code-server@$USER"
        )
    ...
    def uninstall(self):
        ...
        self.execute_command(
            f"sudo apt remove -y code-server",
        ) 
        self.execute_command(
            f"sudo rm -rf /home/featurize/.config/code-server/config.yaml"
        ) 
        self.execute_command(
